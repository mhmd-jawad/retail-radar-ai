...
create table if not exists whatsapp.registered_phones (
...
create index if not exists idx_wa_conv_phone
    on whatsapp.conversations (phone_number, last_message_at desc);
...
create index if not exists idx_wa_promote_pending
    on whatsapp.promote_notifications (tenant_id, outcome, expires_at)
    where outcome = 'pending';

create table if not exists whatsapp.alert_dispatch_log (
    id                uuid primary key default gen_random_uuid(),
    tenant_id         uuid not null references core.tenants(id) on delete cascade,
    ...
    unique (alert_fingerprint, phone_number)
);
